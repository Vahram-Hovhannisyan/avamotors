...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TZVW9XFB"
...
<nav class="navbar">
    <div class="navbar-inner">
        ...
        <a href="{{ route('catalog') }}?brands=1" class="nav-link {{ request()->is('brands*') ? 'active' : '' }}">Бренды</a>
                ...
                <div class="mega-col">
                    @foreach($colGroup as $cat)
                        <div style="margin-bottom:1.4rem;">
                            <a href="{{ route('catalog.category', $cat->slug) }}" class="mega-col-title" onclick="closeMega()">
                                {{ $cat->name }}
                            </a>
                            @if($cat->children->isNotEmpty())
                                @foreach($cat->children->sortBy('sort_order') as $child)
                                    <a href="{{ route('catalog.category', $child->slug) }}" class="mega-link" onclick="closeMega()">
                                        {{ $child->name }}
                                    </a>
                                @endforeach
                            @else
                                <a href="{{ route('catalog.category', $cat->slug) }}" class="mega-link" onclick="closeMega()">Все товары</a>
                            @endif
                        </div>
                    @endforeach
                </div>
