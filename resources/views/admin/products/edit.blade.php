...
@section('content')
    ...
    <div class="tabs">
        ...
        <button class="tab-btn {{ request('tab') === 'analogs' ? 'active' : '' }}" data-tab="analogs">
            🔄 Аналоги
            <span
                class="tab-badge {{ $product->analogs->count() === 0 ? 'empty' : '' }}">{{ $product->analogs->count() }}</span>
        </button>
    </div>
                    ...
                    <div class="form-card">
                        ...
                        <div class="makes-accordion">
                            @foreach($carMakes as $make)
                                ...
                                @endphp
                                <div class="make-group {{ $selectedCount > 0 ? 'open' : '' }}"
                                     data-make="{{ strtolower($make->name) }}">
                                    <div class="make-accordion-trigger {{ $selectedCount > 0 ? 'has-selected' : '' }}"
                                         data-target="make-edit-{{ $make->id }}">
                                        ...
                                        <span class="make-accordion-meta">
                                            ...
                                            <span class="make-selected-badge"
                                                  id="badge-edit-{{ $make->id }}" {{ $selectedCount === 0 ? 'style="display:none"' : '' }}>{{ $selectedCount }}</span>
                                        </span>
                                        <span class="make-accordion-arrow">
                                            <svg viewBox="0 0 10 10" fill="none" stroke="currentColor"
                                                 stroke-width="1.8" stroke-linecap="round"><polyline
                                                    points="2,3.5 5,6.5 8,3.5"/></svg>
                                        </span>
                                    </div>
                                    <div class="models-list {{ $selectedCount > 0 ? 'open' : '' }}"
                                         id="make-edit-{{ $make->id }}">
        ...
        <div class="analogs-layout">
            ...
            <div class="analog-card">
                ...
                <table class="analog-table">
                    ...
                    @forelse($product->analogs as $a)
                        <tr>
                            ...
                            <td class="analog-td-right">
                                <form method="POST"
                                      action="{{ route('admin.products.analogs.detach', [$product, $a]) }}">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="empty-analog">Аналоги не привязаны</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            <div class="analog-card">
                ...
                <table class="analog-table">
                    ...
                    @forelse($suggestions as $s)
                        <tr>
                            ...
                            <td class="analog-td-right-lg">
                                <form method="POST"
                                      action="{{ route('admin.products.analogs.attach', [$product, $s]) }}">
                                    @csrf
                                    <input type="hidden" name="tab" value="analogs">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="empty-analog">
                ...
                @if($suggestions->hasPages())
                    <div
                        class="analog-footer">{{ $suggestions->appends(['tab' => 'analogs', 'q' => request('q')])->links() }}</div>
                @endif
                <div class="analog-footer">
                    <a href="{{ route('admin.analogs.create') }}" target="_blank">+ Создать новый аналог в справочнике
                        →</a>
                </div>
            </div>
